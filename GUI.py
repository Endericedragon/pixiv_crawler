...
class App:
    ...
    # 构造函数, 一些常量和主窗口在此定义.
    def __init__(self):
        self.height, self.width = 12, 5

        self.win = tk.Tk()
        self.win.title('Pixiv Crawler')
        self.win.config(background='#787878')
        self.win.geometry(f'{21 + 250 * self.width:d}x800')
    ...
    def create_widget(self):
        ...
        self.option_menu.add_command(label='Show R18', command=self.r18_on)
    ...
    def __get_total_page(self):
        ...
        if self.show_R18 == 0:
            ...
            self.total_page = [x for x in temp][0][0]
        if self.total_page % 60:
            self.total_page = self.total_page // 60 + 1
        else:
            self.total_page //= 60
